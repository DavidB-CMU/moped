...
The implementations and data are mostly taken from scipy.sandbox.image .

The CIE XYZ tristimulus colorspace with a standard D65 whitepoint is the default
interchange color space for the implementations here. This is a useful
whitepoint for viewing on computer monitors. However, it should be noted that
the dimmer D50 whitepoint is often used in print applications. Notably, ICC
profiles use the XYZ space with a D50 whitepoint as one of its standard
interchange color spaces.
...
import numpy as np
from numpy.dual import inv, solve
...
def separate_colors(xyz, axis=-1):
    """ Separate an array of color triples into three arrays, one for each color
    axis.
...
def adapt_whitepoint(src, dst):
    ...
    if key not in adapt_whitepoint.cache:
        ...
        adapt_whitepoint.cache[key] = solve(bradford, 
...
# normalized to 1.
lms_from_xyz = np.array([[ 0.2434974736455316, 0.8523911562030849, -0.0515994646411065],
                         [-0.3958579552426224, 1.1655483851630273,  0.0837969419671409],
                         [ 0.                , 0.                ,  0.6185822095756526]])
...
def xyz2lab(xyz, axis=-1, wp=whitepoints['D65'][-1]):
    ...
    doclip : bool, optional
        If True, clip the convert L*a*b* values to the defined extrema of the
        L*a*b* space.
...
#  sR'G'B' coordinate system for your computer, and so the rgbp2rgb will put you
#  in the linear coordinate system (assuming normalized to [0,1] sR'G'B'
#  coordiates)
#
